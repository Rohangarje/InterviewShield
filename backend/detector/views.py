...
def session_logs(request):
    """Get paginated session detection logs for the current user"""
    user_id = request.session.get('user_id')
    if not user_id:
        return JsonResponse({'error': 'Authentication required'}, status=401)
    ...
    if status_filter:
        if status_filter in ['high', 'medium', 'low']:
            logs = logs.filter(risk_score=status_filter)
        else:
            logs = logs.filter(status__startswith=status_filter)
    ...
    for log in logs:
        data.append({
            ...
            'phone_confidence': log.phone_confidence,
